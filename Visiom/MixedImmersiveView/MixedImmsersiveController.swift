...
extension MixedImmersiveController {
    ...
    private func spawnEntity(_ anchorRecord: AnchorRecord) async {
...
extension MixedImmersiveController {
    ...
    /// 엔티티 계층 구조 업데이트
    private func updateEntityHierarchy() {
        guard let root else { return }
    ...
    private func updateGroupVisibility() {
        photoGroup?.isEnabled = true
        memoGroup?.isEnabled = true
        teleportGroup?.isEnabled = true
    }
}
